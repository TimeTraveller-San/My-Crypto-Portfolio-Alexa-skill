'''
My crypto portfolio data API developed by

Siddhant Bhambri: https://github.com/pickle0412
Utsav dahiya: https://github.com/utsavdahiya
Vaibhav Kumar: https://github.com/TimeTraveller-San
...
def get_index(date):
    if not date :
        ...
        print("DAYS:",(date - current_date).days)
...
def get_prediction(crypto, date):
    ...
    for line in lines:
        elems = line.split(',')
        if line and elems[1] == str(ts):
            prediction = elems[2]
    if prediction:
        if crypto == 'doge':
            price = "{:.5f}".format(float(prediction))
        else:
...
# Following are some functions you can run:
